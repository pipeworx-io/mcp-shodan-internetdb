...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/shodan_internetdb_lookup_ip \
  -H 'Content-Type: application/json' \
  -d '{"ip":"8.8.8.8"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/shodan_internetdb_lookup_ip`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "shodan-internetdb": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-shodan-internetdb"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-shodan-internetdb
```
